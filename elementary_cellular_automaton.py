...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  9 12:25:24 2017
...
pattern = {1:'...',2:'..x',4:'.x.',8:'.xx',16:'x..',32:'x.x',64:'xx.',128:'xxx'}
...
def cellular_automaton(string,rules_int,generation):
    current = string
    rules_str = create_rules(rules_int)
    if generation <= 0:
        return current
    else:
        next = ''
        for a in range(0,len(current)):
            if a + 1 > len(current)  - 1:
                c = 0
            else:
                c = a + 1
            mini_string = current[a-1] + current[a] + current[c]
            if mini_string in rules_str:
                next += 'x'
            else:
                next += '.'
        generation += -1
        return cellular_automaton(next,rules_int,generation)
