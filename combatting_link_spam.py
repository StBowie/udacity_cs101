...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb  8 00:01:24 2017
...
def reciprocal_link(graph,k):
    recip = {}
    for page in graph:
        recip[page] = [page]
        for a in range(0,k):
            for x in recip[page]:
                recip[page] = recip[page] + graph[x]
    return recip
...
def compute_ranks(graph,k):
    ...
    for i in range(0, numloops):
        ...
        for page in graph:
            newrank = (1 - d) / npages
            for node in graph:
                if page in graph[node] and node not in recip[page]:
